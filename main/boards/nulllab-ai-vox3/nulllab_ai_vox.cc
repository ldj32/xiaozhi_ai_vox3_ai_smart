...
private:
    ...
    void InitializeLcdDisplay()
    {
        ...
        display_ = new SpiLcdDisplay(panel_io,
                                     panel,
                                     DISPLAY_WIDTH,
                                     DISPLAY_HEIGHT,
                                     DISPLAY_OFFSET_X,
    ...
    void InitializeUart()
    {
        uart_config_t uart_config = {
            .baud_rate = DEVICE_UART_BAUD_RATE,
        ...
        ESP_ERROR_CHECK(uart_driver_install(DEVICE_UART_NUM, 1024, 0, 0, NULL, 0));
        ESP_ERROR_CHECK(uart_param_config(DEVICE_UART_NUM, &uart_config));
        ESP_ERROR_CHECK(uart_set_pin(DEVICE_UART_NUM, DEVICE_UART_TX_PIN, DEVICE_UART_RX_PIN, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));
        uart_port_ = DEVICE_UART_NUM;
    }

    void RegisterUartMcpTool()
    {
        ...
        mcp_server.AddTool("self.uart.send",
                           ...
                           [this](const PropertyList &properties) -> ReturnValue
                           {
                               ...
                               if (uart_write_bytes(uart_port_, data.c_str(), data.length()) == data.length())
                               {
        ...
        ESP_LOGI(TAG, "UART MCP tool registered");
    }
...
public:
          ...
          volume_down_button_(VOLUME_DOWN_BUTTON_GPIO, VOLUME_DOWN_BUTTON_EN_INVERT)
    {
        ...
        display_->SetChatMessage("system", "AI语音助手");
    }
    ...
    {
        static AIVoxAudioCodec audio_codec(codec_i2c_bus_,
                                           I2C_NUM_0,
